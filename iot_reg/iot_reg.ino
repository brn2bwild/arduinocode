...
bool registroCliente(){
	...
	String data = "ip="+WiFi.localIP().toString()+"&mac="+String(WiFi.macAddress());
...
void configServer(int tipoWeb){
	if(tipoWeb == 1){ //si la opcion es 1 indica que el servidor esta con el dispositivo en modo AP
		server.on("/", [](){ //si la opcion es 0 indica que el servidor esta con el dispositivo conectado a la red local
			...
			contenido = "<!DOCTYPE HTML><html><form method='get' action='setting'><head><style>.center-input{width:100%;font-size:40px;margin:0 auto;}";
			contenido += ".center-label{font-size:40px;width:30%;margin:0 auto;height: 300px;}</style><meta name='viewport' content='width=device-width,initial-scale=1.0'>";
			contenido += "<meta charset='utf-8'><title>IOT Device</title></head><body><span class='center-input'>Introduzca las credenciales</span><br>";
			contenido += lista;
			contenido += "<label class='center-label'>SSID:</label><input class='center-input' name='ssid' length=32><br>";
			contenido += "<label class='center-label'>PASS:</label><input class='center-input' name='pass' length=32><br>";
			contenido += "<label class='center-label'>HOST:</label><input class='center-input' name='host' length=32><br>";
			contenido += "<label class='center-label'>ID:</label><input class='center-input' name='id' length=10><br>";
			contenido += "<input class='center-input' type='submit'></body></form></html>";
		...
		server.on("/reset_disp", [](){
			...
			contenido += "<span style='font-size: 40px'>EEPROM borrada</span><br>";
			contenido += "<span style='font-size: 40px'>Credenciales WiFi borradas</span></html>";
			server.send(200,"text/html",contenido);
