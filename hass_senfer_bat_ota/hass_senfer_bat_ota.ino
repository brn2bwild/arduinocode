...
void setup() {
  ...
  if(config.configurado){
    ...
    Serial.println(config.userMQTT);Serial.println(config.passMQTT);
  ...
  if(boton_presionado_inicio){
    ...
    if(wifi_ota){
      ...
      if(Client.connect(config.MAC, config.userMQTT, config.passMQTT)){
...
void enviarDatos(){  
  if(Client.connect(config.MAC, config.userMQTT, config.passMQTT)){
    ...
    Client.publish(config.topicoSondaA, dato, true);

    snprintf(dato, 8, "%.1f", sondasTemp.getTempCByIndex(1));
    Client.publish(config.topicoSondaB, dato, true);

    snprintf(dato, 8, "%.1f", sondasTemp.getTempCByIndex(2));
    Client.publish(config.topicoSondaC, dato, true);

    snprintf(dato, 8, "%d", obtenerVBAT());
    Client.publish(config.topicoBateria, dato, true);
