...
void loop() {
  ...
  if(disp_Configurado){
    ...
    float sensacion = sensorAmbiente.computeHeatIndex(temperatura, humedad, false);
    ...
    // http.begin(host,80,"https://nist.time.gov/actualtime.cgi");
    // int httpCode = http.GET();
    ...
    Firebase.setFloat("tempAire/"+folio+"/valor/",temperatura);
    Firebase.set("tempAire/"+folio+"/hora/",now);
    Firebase.setFloat("humedadAire/"+folio+"/valor/",humedad);
