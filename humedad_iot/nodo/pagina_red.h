...
<tr><td align="center">SSID:</td><td><input type="text" id="ssid" name="ssid" value="" maxlength="32"></td></tr>
<tr><td align="center">Password:</td><td><input type="password" id="password" name="password" value="" maxlength="32"></td></tr>
</table>
<br>
...
void send_network_configuration_html(){
	...
	if (Server.args() > 0 ){  // Save Settings
		...
		for ( uint8_t i = 0; i < Server.args(); i++ ) {
			if(Server.argName(i) == "ssid") urldecode(Server.arg(i), config.ssid, sizeof(config.ssid));
			if(Server.argName(i) == "password") urldecode(Server.arg(i), config.password, sizeof(config.password));
