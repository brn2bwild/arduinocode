...
<table align="center">
<tr><td align="center">Nombre:</td><td><input type="text" id="devicename" name="devicename" value="" maxlength="6"></td></tr>
<tr><td align="center">Gateway:</td><td><input type="text" id="gateway" name="gateway" value="" maxlength="8"></td></tr>
...
void send_general_html(){
	if (Server.args() > 0 ){  // Save Settings
		...
		for ( uint8_t i = 0; i < Server.args(); i++ ) {
			...
			if (Server.argName(i) == "gateway") urldecode(Server.arg(i), config.gateway, sizeof(config.gateway));
			if (Server.argName(i) == "clave") urldecode(Server.arg(i), config.clave, sizeof(config.clave));//config.puertoMQTT = Server.arg(i).toInt();
...
void send_general_configuration_values_html(){
	String values ="";
	values += "devicename|" + (String) config.dispID + "|input\n";
    values += "gateway|" +  (String) config.gateway + "|input\n";
