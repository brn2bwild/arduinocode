...
const char* mqtt_server = "192.168.0.200";  //se debe configurar la ip del raspberry para que quede estática
//const char* mqtt_server = "iot.eclipse.org";  //se debe configurar la ip del raspberry para que quede estática
const int mqtt_port = 1883;
const char* IDCliente = "sT0100003";   // cadena que sirve para que los dispositivos se comuniquen con el broker de manera individual
...
void indicarOrden(char orden){
  char jsonMensaje[80];
  if(orden == 'r'){
    snprintf(jsonMensaje, 80, "{\"dis\":\"%02X:%02X:%02X:%02X:%02X:%02X\",\"tipo\":\"%s\",\"estado\":\"reiniciado\"}", MAC_WIFI[0], MAC_WIFI[1], MAC_WIFI[2], MAC_WIFI[3], MAC_WIFI[4], MAC_WIFI[5], tipoSensor);
    client.publish("sistema/dispositivos/sensores",jsonMensaje);
    delay(1000);
    ESP.restart();
  }else if(orden == 'd'){
    snprintf(jsonMensaje, 80, "{\"dis\":\"%02X:%02X:%02X:%02X:%02X:%02X\",\"tipo\":\"%s\",\"estado\":\"deshabilitado\"}", MAC_WIFI[0], MAC_WIFI[1], MAC_WIFI[2], MAC_WIFI[3], MAC_WIFI[4], MAC_WIFI[5], tipoSensor);
  }else if(orden == 'h'){
    snprintf(jsonMensaje, 80, "{\"dis\":\"%02X:%02X:%02X:%02X:%02X:%02X\",\"tipo\":\"%s\",\"estado\":\"activo\"}", MAC_WIFI[0], MAC_WIFI[1], MAC_WIFI[2], MAC_WIFI[3], MAC_WIFI[4], MAC_WIFI[5], tipoSensor);
  }
  client.publish("sistema/dispositivos/sensores",jsonMensaje);
  delay(1000);
}
...
void conectar_Servidor() {
  ...
  if(WiFi.status() == WL_CONNECTED){
    if (!client.connected()) {
      ...
      snprintf(mensajeLastwill, 80, "{\"dis\":\"%02X:%02X:%02X:%02X:%02X:%02X\",\"tipo\":\"%s\",\"estado\":\"inactivo\"}", MAC_WIFI[0], MAC_WIFI[1], MAC_WIFI[2], MAC_WIFI[3], MAC_WIFI[4], MAC_WIFI[5],tipoSensor);
  ...
  if((unsigned long)(ahora - tiempoMensajeAnterior) >= intervaloMensaje){
    ...
    if((estadoWiFi == 1) && (WiFi.status() == WL_CONNECTED)){
      if(!client.connected()){
        ...
        snprintf(jsonMensaje, 180, "{\"dis\":\"%02X:%02X:%02X:%02X:%02X:%02X\",\"temSnd\":%.1f,\"temObj\":%.1f,\"temAir\":%.1f,\"humAir\":%.1f,\"senTer\":%.1f,\"punRoc\":%.1f,\"preAtm\":%.1f}", MAC_WIFI[0], MAC_WIFI[1], MAC_WIFI[2], MAC_WIFI[3], MAC_WIFI[4], MAC_WIFI[5], temSondaC, temObjetoC, temAireC, humAire, senAire, puntoRocioC, presionAtm);
